...
Values are stored as strings — always, losslessly, so a save round-trips exactly and `02134` is never silently mangled into `2134`. There are four types: string, number, boolean, and date. A cell is a string until an explicit cast (`num`, `bool`, `date`) commits it to a type at the point of arithmetic or logic. The date cast carries the same refusal one step further: it reads ISO on its own and demands the layout be spelled out for anything else, because deciding between the two readings of `03/04/2024` is exactly the silent guess this model exists to prevent. awk's own "string unless used as a number" philosophy is close to this and is on our side; the one deliberate departure is requiring the explicit cast rather than inheriting awk's implicit coercion, to keep leading zeros and ID-like fields safe. Recompute and stored formulas are explicitly out of scope; a computed value is written once, not maintained as a live dependency.
